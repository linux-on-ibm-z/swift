...
static inline void storeValue(uint8_t *dst, uint32_t value, size_t size) {
  switch (size) {
  case 0:
    return;
  case 1:
...
static inline uint32_t loadValue(const uint8_t *src, size_t size) {
  uint32_t result = 0;
  switch (size) {
  case 0:
    return 0;
...
inline unsigned getEnumTagSinglePayloadImpl(
  ...
  if (emptyCases > payloadNumExtraInhabitants) {
    ...
    unsigned extraTagBits = loadValue(extraTagBitAddr, numBytes);
    ...
    if (extraTagBits > 0) {
      ...
      unsigned caseIndexFromValue = loadValue(valueAddr, payloadSize);
...
inline void storeEnumTagSinglePayloadImpl(
  ...
  if (whichCase <= payloadNumExtraInhabitants) {
    if (numExtraTagBytes != 0)
      storeValue(extraTagBitAddr, 0, numExtraTagBytes);
  ...
  if (payloadSize)
    storeValue(valueAddr, payloadIndex, payloadSize);
  if (numExtraTagBytes)
    storeValue(extraTagBitAddr, extraTagIndex, numExtraTagBytes);
}

...
static void storeMultiPayloadTag(OpaqueValue *value,
  ...
  write_tag(tagBytes, layout.numTagBytes, tag, layout.numTagBytes);
}
...
static void storeMultiPayloadValue(OpaqueValue *value,
  ...
  auto size = std::min(layout.payloadSize, sizeof(payloadValue));
  write_native_byte_order(bytes, layout.payloadSize,
                          payloadValue, size,
                          /* zeroUnused = */ true);
}
...
static unsigned loadMultiPayloadTag(const OpaqueValue *value,
  ...
  return read_tag(tagBytes, layout.numTagBytes, layout.numTagBytes);
}

static unsigned loadMultiPayloadValue(const OpaqueValue *value,
                                      MultiPayloadLayout layout) {
  auto bytes = reinterpret_cast<const uint8_t *>(value);
  return read_native_byte_order(bytes, layout.payloadSize,
                                std::min(layout.payloadSize, sizeof(uint32_t)));
}
